...
if (defined $Config) {
	foreach my $neType (keys %simNesCountMap){
		...
		#$ipv6Map{$neType} = $resIpv6, $numOfIpv6Nes+=$resIpv6 if $resIpv6 != 0;
		if ($resIpv6 != 0) {
			$resIpv6 *= $neTypesIpMultiplier{$neType} if exists $neTypesIpMultiplier{$neType};
			$numOfIpv6Nes+=$resIpv6; 
			$ipv6Map{$neType} = $resIpv6; 
		}
	 	my $resIpv4 = $numOfNes - $resIpv6;	
		#$ipv4Map{$neType} = $resIpv4, $numOfIpv4Nes+=$resIpv4 if $resIpv4 != 0;
 		if ($resIpv4 != 0){
			$resIpv4 *= $neTypesIpMultiplier{$neType} if exists $neTypesIpMultiplier{$neType};
