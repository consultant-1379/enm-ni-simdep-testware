...
simdepUnzipPath=/netsim/tls/enm-ni-simdep/
...
su - netsim -c "unzip /netsim/ERICTAFenmnisimdep_CXP9031884-${SIMDEP_RELEASE}.jar -d $simdepUnzipPath";
...
echo "[INFO] Running ./removeTLS.sh to remove TLS certificates on the existing radio nodes."
su - netsim -c "cd ${workingPath}utils/; ./removeTLS.sh"
catchError "ERROR: Failed to run removeTLS.sh script."

ssh-keygen -R $MANAGEMENT_SERVER >/dev/null 2>&1

echo "[INFO] Turning TLS ON"
confPath=${workingPath}conf/conf.txt
su - netsim -c "sed -i '/SETUP_SECURITY_TLS/c\SETUP_SECURITY_TLS=ON' $confPath"
catchError "ERROR: Failed to enable TLS on."
su - netsim -c "sed -i '/SETUP_SECURITY_SL2/c\SETUP_SECURITY_SL2=OFF' $confPath"
catchError "ERROR: Failed to disable SL2 off."


echo "[INFO] Re-applying Security Definition for DG2|LTE PICO nodes"
