...
python /var/tmp/runCliCommand.py 'pkiadm extcalist' $ENM_URL > $externalCertsStatusLog
if [[ $? -ne 0 ]]
then
...
if [ -z "$trustStatus" ]; then
	...
	if [ -z "$extCAStatus" ] ; then
        ...
        #################################################
       python /var/tmp/runCliCommand.py 'pkiadm extcaimport -fn file:s_cacert.pem --chainrequired false --name "ENM_ExtCA3"' $ENM_URL $dirSimNetDeployer/s_cacert.pem >> $trustProfileLog
        if [[ $? -ne 0 ]]
        then
   ...
   cp  ${SCRIPT_DIR}/trustProfile.xml .
    ...
    #################################################
    python /var/tmp/runCliCommand.py 'pkiadm profilemgmt --export --profiletype trust --name ENM_SBI_FCTP_TP' $ENM_URL downloadFile /var/tmp/Certs/existingProfileData.xml
    sed 'x;$ e cat trustProfile.xml' /var/tmp/Certs/existingProfileData.xml | sed '$s/$/\n<\/Profiles>/;/^$/d' > trustProfileUpdate.xml
    

    python /var/tmp/runCliCommand.py 'pkiadm pfm -u -xf file:trustProfileUpdate.xml' $ENM_URL $dirSimNetDeployer/trustProfileUpdate.xml >> $trustProfileLog
    if [[ $? -ne 0 ]]
    then
    ...
    python /var/tmp/runCliCommand.py 'pkiadm extcalist' $ENM_URL
    if [[ $? -ne 0 ]]
    then
        echo "ERROR: Executing pkiadm extcalist command failed."
        exit 201
