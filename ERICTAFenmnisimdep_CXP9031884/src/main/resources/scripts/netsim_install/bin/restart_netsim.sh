...
function message ()
{
    ...
    then
        COLOR=$red
        MESSAGE="$1 in module restart_netsim.sh "
    fi
    if [[ "$TYPE" == "LINE" ]]
    then
        COLOR=$magenta
...
if [[ -z "$logFile" ]];
then
   export logFile="$(pwd)/../log/`date +%Y_%m_%d_%H_%M_%S`_restart_netsim.log"
   Flag=1
fi
...
then
    ...
    then
        ...
        message "ERROR: Only root user can execute the restart_netsim.sh!           -" ERROR
        message "ERROR: ==================== ERROR!!! ===============================" ERROR
        exit 208
    fi
fi
...
$MOUNTPOINT/stop_netsim.sh -m $MOUNTPOINT
    if [[ ${PIPESTATUS[0]} -ne 0 ]]
    then
        message "ERROR: stop_netsim failed to execute" ERROR
        exit 207
    fi
$MOUNTPOINT/start_netsim.sh -m $MOUNTPOINT
    if [[ ${PIPESTATUS[0]} -ne 0 ]]
    then
        message "ERROR: start_netsim.sh failed to execute" ERROR
        exit 207
    fi
